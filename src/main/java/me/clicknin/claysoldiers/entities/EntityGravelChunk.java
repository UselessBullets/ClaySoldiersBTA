...
public class EntityGravelChunk extends Entity {
    ...
    public EntityGravelChunk(World world, EntityLiving entityliving, int i) {
        ...
        this.setSize(0.1F, 0.1F);
        this.renderDistanceWeight = 5.0D;
        this.setLocationAndAngles(entityliving.x, entityliving.y + (double)entityliving.getHeadHeight(), entityliving.z, entityliving.rotationYaw, entityliving.rotationPitch);
        this.x -= (double)(MathHelper.cos(this.rotationYaw / 180.0F * 3.141593F) * 0.16F);
        this.y -= (double)0.1F;
        this.z -= (double)(MathHelper.sin(this.rotationYaw / 180.0F * 3.141593F) * 0.16F);
        this.setPosition(this.x, this.y, this.z);
        this.yOffset = 0.0F;
        this.xd = (double)(-MathHelper.sin(this.rotationYaw / 180.0F * 3.141593F) * MathHelper.cos(this.rotationPitch / 180.0F * 3.141593F));
        this.zd = (double)(MathHelper.cos(this.rotationYaw / 180.0F * 3.141593F) * MathHelper.cos(this.rotationPitch / 180.0F * 3.141593F));
        this.yd = (double)(-MathHelper.sin(this.rotationPitch / 180.0F * 3.141593F));
    ...
    public void setVelocity(double d, double d1, double d2) {
        ...
        if(this.prevRotationPitch == 0.0F && this.prevRotationYaw == 0.0F) {
            ...
            this.setLocationAndAngles(this.x, this.y, this.z, this.rotationYaw, this.rotationPitch);
            this.ticksInGround = 0;
        }
    ...
    public void onUpdate() {
        ...
        if(this.prevRotationPitch == 0.0F && this.prevRotationYaw == 0.0F) {
            ...
            this.prevRotationYaw = this.rotationYaw = (float)(Math.atan2(this.xd, this.zd) * 180.0D / (double)(float)Math.PI);
            this.prevRotationPitch = this.rotationPitch = (float)(Math.atan2(this.yd, (double)i) * 180.0D / (double)(float)Math.PI);
        }

        int i18 = this.world.getid(this.xTile, this.yTile, this.zTile);
        if(i18 > 0) {
            Block.blocksList[i18].setBlockBoundsBasedOnState(this.world, this.xTile, this.yTile, this.zTile);
            AxisAlignedBB vec3d = Block.blocksList[i18].getCollisionbbFromPool(this.world, this.xTile, this.yTile, this.zTile);
        ...
        if(this.inGround) {
            int i20 = this.world.getid(this.xTile, this.yTile, this.zTile);
            ...
            MovingObjectPosition movingobjectposition = this.world.func_28105_a(vec3D19, vec3d1, false, true);
            ...
            for(int f2 = 0; f2 < list.size(); ++f2) {
                ...
                if(f3.canBeCollidedWith() && (f3 != this.owner || this.ticksInAir >= 5)) {
                    f5 = 0.3F;
                    AxisAlignedBB a = f3.bb.expand((double)f5, (double)f5, (double)f5);
            ...
            if(movingobjectposition != null) {
                if(movingobjectposition.entityHit != null) {
                    ...
                    this.inTile = this.world.getid(this.xTile, this.yTile, this.zTile);
                    this.field_28019_h = this.world.getBlockMetadata(this.xTile, this.yTile, this.zTile);
                    this.xd = (double)((float)(movingobjectposition.hitVec.xCoord - this.x));
                    this.yd = (double)((float)(movingobjectposition.hitVec.yCoord - this.y));
                    this.zd = (double)((float)(movingobjectposition.hitVec.zCoord - this.z));
            ...
            this.rotationYaw = (float)(Math.atan2(this.xd, this.zd) * 180.0D / (double)(float)Math.PI);

            for(this.rotationPitch = (float)(Math.atan2(this.yd, (double)f24) * 180.0D / (double)(float)Math.PI); this.rotationPitch - this.prevRotationPitch < -180.0F; this.prevRotationPitch -= 360.0F) {
            }
            ...
            if(this.isDead) {
                ...
                Minecraft.getMinecraft().effectRenderer.addEffect(new EntityDiggingFX(this.world, d26, b, c, 0.0D, 0.0D, 0.0D, Block.gravel, 0, 0));
    ...
    public void onCollideWithPlayer(EntityPlayer entityplayer) {
        if(!this.world.isMultiplayerandomNotHost) {
